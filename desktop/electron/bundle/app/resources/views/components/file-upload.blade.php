@props(['accept' => null, 'multiple' => false, 'error' => null])
...
<div x-data="{ files: [], dragging: false }" 
     ...
     :class="dragging ? 'border-primary-400 bg-primary-50' : ''">
    <div class="space-y-1 text-center">
        ...
        <div class="flex text-sm text-gray-600">
            <label class="relative cursor-pointer bg-white rounded-md font-medium text-primary-600 hover:text-primary-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary-500">
                ...
                <input type="file" 
                       {{ $attributes->merge(['class' => 'sr-only', 'accept' => $accept, 'multiple' => $multiple]) }}
                       @change="files = Array.from($event.target.files)">
            </label>
            <p class="pl-1">or drag and drop</p>
        </div>
        <p class="text-xs text-gray-500">PNG, JPG, PDF up to 10MB</p>
        <div x-show="files.length > 0" class="mt-2 text-sm text-gray-900">
...
@if($error)
    <p class="mt-1 form-error">{{ $error }}</p>
@endif
